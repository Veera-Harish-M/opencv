...
erroded=cv2.erode(np.array(new_img,dtype=np.uint8),kernel, iterations=1)

dilated=cv2.dilate(np.array(new_img,dtype=np.uint8),kernel, iterations=1)
cv2.imshow("eroded",np.array(erroded,dtype=np.uint8))
cv2.imshow("dilated",np.array(dilated,dtype=np.uint8))
plt.show()
cv2.waitKey(0)
